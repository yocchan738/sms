...
@WebServlet(urlPatterns = {"/subjectmanagement/update/commit"})
public class SubjectUpdateCommit extends HttpServlet{
    ...
    public void doPost(
        ...
        try {
            ...
            // 挿入成功の場合、成功画面に遷移
            if (line > 0) {
                request.getRequestDispatcher("/subject_management/subject_update_commit.jsp").forward(request, response);
            }
        } catch (Exception e) {
            // 例外が発生した場合、失敗画面に遷移
