...
public class StudentDAO extends DAO {
	 ...
	 * @return List<Bean> 検索結果のリスト
	 */
	public List<Bean> searchScoresID(int keyword) throws Exception{
		List<Bean> list=new ArrayList<>();
		
		Connection con=getConnection();
		
		PreparedStatement st=con.prepareStatement(
				"SELECT * FROM scores sc JOIN student s ON sc.student_id = s.student_id JOIN class c ON s.class_id = c.class_id JOIN school sch ON s.school_id = sch.school_id JOIN subjects sub ON sc.subject_id = sub.subject_id JOIN teacher t ON sub.teacher_id = t.teacher_id where sc.score_id=?");
	 ...
	 * @return List<Bean> 検索結果のリスト
	 */
	public List<Bean> searchStudentID(int keyword) throws Exception{
		List<Bean> list=new ArrayList<>();
		
		Connection con=getConnection();
		
		PreparedStatement st=con.prepareStatement(
				"SELECT * FROM student s join class c on s.class_id=c.class_id join school sch on s.school_id=sch.school_id where s.student_id = ?");
